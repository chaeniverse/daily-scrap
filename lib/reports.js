...
function cleanReportText(md) {
  const lines = md.split("\n").filter((l) => {
    ...
    if (/Carta 데이터/.test(l)) return false; // Carta 관련 안내 줄
    return true;
  });
  let out = lines.join("\n");
...
function parseFile(file) {
  ...
  let shortDate = displayDate;
  let month = "";
  if (/^\d{4}-\d{2}-\d{2}$/.test(fileDate)) {
    const [y, m, d] = fileDate.split("-");
    const wd = WD[new Date(`${fileDate}T00:00:00`).getDay()];
    shortDate = `${Number(m)}월 ${Number(d)}일 (${wd})`;
  ...
  return {
    ...
    displayDate,              // 화면 표시용
    shortDate,                // 그리드용 (M월 D일 (요일))
    month,                    // 월별 묶음용 (YYYY년 M월)
    title,
